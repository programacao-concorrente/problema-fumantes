//esboço de solução, acrescentando o incremento dos contadores (sem guarda) 
//esboco-solucao-contadores-sem-guarda.c
...
void process_agent(){
	while(true){
		...
		while(qtdAgenteBotouIngredientes < (qtdSmokerTobaccoFummou + qtdSmokerPaperFumou + qtdSmokerMatchFumou)) {skip}
...
void process_smoker1(){
	while(true){
		while(vst > qtdSmokerTobaccoFummou) {skip}
...
void process_smoker2(){
	while(true){
		while(vsp > qtdSmokerPaperFumou) {skip}
...
void process_smoker3(){
	while(true){
		while(vsm > qtdSmokerMatchFumou) {skip}
